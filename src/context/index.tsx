...
export interface UserProviderData {
    ...
    recoverPasswordUpdatePassword: (
        data: iRecoverPasswordUpdatePassword
    ) => any; // era void, agora é any para teste de deploy
...
function ContextDadosUser({ children }: iInfoUser) {
    ...
    const registerUser = async (data: iUserRegisterReq) => {
        ...
        const responseAdress = await instanceAxios.post(
            "adress",
            registerAdress
        );
    ...
    const updateUser = async (data: iUserUpate) => {
        if (token) {
            const responseUser = await instanceAxios.patch(`user`, {
                ...infoUserLogin,
                ...data,
            });
    ...
    const getIdAdressUser = async () => {
        if (token && idAdressUser !== "") {
            const responseAdress = await instanceAxios.get(
                `adress/${idAdressUser}`
            );
    ...
    const patchAdressUser = async (data: iAddressUpdate): Promise<any> => {
        if (token) {
            const responseAdress = await instanceAxios.patch(`adress`, {
                ...dataAdress,
                ...data,
            });
    ...
    const recoverPasswordSendEmail = async (
        emailData: iRecoveriPasswordSendEmail
    ) => {
        const responseToeknRecoverPassword = await instanceAxios.post(
            `forgot/pass`,
            emailData
        );
    ...
    const recoverPasswordUpdatePassword = async (
        passwordData: iRecoverPasswordUpdatePassword
    ) => {
        await instanceAxios.patch(
            `forgot/pass/${tokenRecoverPassword}`,
            passwordData
        );
    };
